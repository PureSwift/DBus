...
/// DBus type code.
///
/// A single ASCII character identifying a D-Bus type, as used in signatures and
/// in the type-code position of a marshalled value.
public enum DBusType: String, Sendable, CaseIterable {
...
public extension DBusType {
    ...
    /// Whether the type is one of the length-prefixed, NUL-terminated string types.
...
internal extension DBusType {
    ...
    /// Initialize from the ASCII value of the type code.
    init?(_ integerValue: UInt8) {

        let scalar = Unicode.Scalar(integerValue)
    ...
    /// The ASCII value of the type code.
    var integerValue: UInt8 {

        return rawValue.utf8.first!
    }
}
