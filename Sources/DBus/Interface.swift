 ...
 */
public struct DBusInterface: Sendable {
...
internal extension DBusInterface {
    ...
    /// Minimum is "a.b", maximum applies to bus names, interfaces and members alike.
    ...
    static func parseThrowing(_ string: String) throws -> [Element] {
        ...
        // Length is measured in bytes, and only ASCII is legal in an interface name.
        guard string.utf8.count >= length.min,
            string.utf8.count <= length.max,
            string.first != separator, // must not begin with '.'
            string.last != separator, // must not end with '.'
            string.contains(separator) // must contain at least one '.'
            else { throw DBusError.invalidInterface(string) }
...
private extension DBusError {
    ...
    static func invalidInterface(_ string: String) -> DBusError {

        return DBusError(name: .invalidArguments, message: "Interface name was not valid: '\(string)'")
    }
}
...
extension DBusInterface: RawRepresentable {
    ...
    public init?(rawValue: String) {
        ...
        guard let elements = try? DBusInterface.parseThrowing(rawValue)
            else { return nil }
...
public extension DBusInterface {
    ...
    /// An element in the interface name
    struct Element: Sendable {
        ...
        internal init?(substring: Substring) {
            ...
            let bytes = substring.utf8

            // No element may be an empty string, must only contain "[A-Z][a-z][0-9]_",
